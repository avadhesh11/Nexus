          ...
          {messages.length === 0 && (
            <div className="text-center py-16">
              ...
              <div className="flex flex-wrap gap-2 justify-center mt-6">
                {["What did we decide about the database?", "Summarize our sprint goals", "Who is handling the migration?"].map(q => (
            ...
            <div key={i} className={`flex gap-3 ${i === messages.length - 1 ? "animate-fade-up" : ""}`}>
              <div className="flex-shrink-0 mt-0.5">
                {msg.role === "user"
                  ...
                  : <div className="w-7 h-7 rounded-lg bg-accent/10 border border-accent/20 flex items-center justify-center"><Sparkles className="w-3.5 h-3.5 text-accent" /></div>
                }
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-semibold">{msg.role === "user" ? (user?.email?.split("@")[0] || "You") : "Nexus AI"}</span>
                  {msg.sources !== undefined && msg.sources > 0 && (
                    <span className="flex items-center gap-1 text-[10px] font-mono text-[#5a5a7a] border border-[#1e1e2e] px-1.5 py-0.5 rounded-full">
                      <Database className="w-2.5 h-2.5" />{msg.sources} sources
                    </span>
                  )}
                </div>
                <div className="text-sm text-[#e8e8f0] leading-relaxed whitespace-pre-wrap">{msg.content}</div>
              </div>
            </div>
          ))}
          ...
          {streamText && (
            <div className="flex gap-3 animate-fade-up">
              ...
              <div className="flex-1">
                <div className="text-xs font-semibold mb-1">Nexus AI</div>
                <div className="text-sm text-[#e8e8f0] leading-relaxed whitespace-pre-wrap">
                  {streamText}
                  <span className="inline-block w-0.5 h-3.5 bg-accent animate-blink ml-0.5 align-middle" />
                </div>
              </div>
            </div>
          )}
